...
spec = do
  ...
  describe "[should semantic error] Parse & Compile & Run" $ do
    ...
    let programNames    = ["no_such_ident", "no_such_ident_callee", "global_type_mismatch", "local_type_mismatch", "if_cond_type_mismatch", "if_then_type_mismatch", "func_body_type_mismatch"] -- NOTE: All you have to do is adding here
    let platyExtension  = "platy"
